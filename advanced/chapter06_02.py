...
# c2.send(100) # 예외 발생 # 제네레이터를 한번이라도 사용해야됨. next(c2)
...
def coroutine2(x):
    ...
    y = yield x # 제네레이터 크리에이티드(만들어지기만)한 상태.
...
print(next(c3)) # y에 10을 보내줌.

print('EX1-3 -', getgeneratorstate(c3)) # 메인에서y를 받았으니 yield대기상태

print(c3.send(15))# 대기상태가 되면 다시 x에 15를 보내고 아까 보낸 10에 15를 더해서 z에 25출력

# print(c3.send(20)) # 예외 , 20은 밑에 yield가 없어서 더이상 동작을 안하고 에러를 발생시키면서 종료 됨.
...
def coroutine(func):
    ...
    @wraps(func) # coroutine(func) 코루틴 함수에 인자로 들어가는 함수에 들어있는 다큐멘트나 여러 가지 정보들을 랩으로 싸서 가지고 오겠다 라는 뜻.
...
def coroutine_except():
    ...
    try:
        while True:
            try:
                x = yield # 메인루틴에 있는 send기능을 통해 들어온 인자로부터 값을 받는다. .send(30)이면 x에 30이 들어가 있고 그걸 yield가 받음. 그다음에 일드 오른쪽에 값이 있으면 그걸 메인루틴으로 보내주는 것.

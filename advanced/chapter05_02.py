...
class VectorP(object):
    # private 선언
    def __init__(self, x, y): # VectorP라는 클래스를 이용해서 인스턴스(v = VectorP(15,20))를 만들 때 걸러주는 기능, 그러나 이 이후에 변수 v.__x = 20 이렇게 적으면 내가 아무리 필터를 만들어도 적용이 안됨.
        self.__x = float(x) # __일 경우 파이썬이 그 클래스 안에서 숨겼으므로 직접접근(v.__x로 값을 불러옴)이 불가능하다.
        self.__y = float(y)

    def __iter__(self):
        return (i for i in (self.__x, self.__y)) # Generator

    # 인스턴스를 선언할 때 1차 필터를 했지만 혹시모를 상황(사람들이 인스턴스 만들때 말고 만들고 나서 임의로 값을 바꿀 수도 있으므로)에 대비하여 2차 필터생성.
    @property 
    def x(self):
        return self.__x
...
class IterTestA():
    def __getitem__(self, idx):
        # return range(1, 50, 2) # 뒤에 인덱스가 없으면 range(1,50,2)이게 그대로 출력됨
